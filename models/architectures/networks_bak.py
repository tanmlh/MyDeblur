...
class ResnetGenerator(nn.Module):
    def __init__(
        ...
        model = [
            ...
            nn.Conv2d(num_in_chns, ngf, kernel_size=7, padding=0, bias=use_bias),
            norm_layer(ngf),
            nn.ReLU(True)
        ]
        ...
        model += [
            nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, bias=use_bias),
            norm_layer(64),
            nn.ReLU(True),
        ...
        model += [
            ...
            nn.ConvTranspose2d(64, 64, kernel_size=3, stride=2, padding=1, output_padding=1, bias=use_bias),
            norm_layer(64),
            nn.ReLU(True),
        ]
        ...
        model += [
            nn.ReflectionPad2d(3),
            nn.Conv2d(64, num_out_chns, kernel_size=7, padding=0),
            nn.Tanh()
        ]
